...
export async function GET(request: NextRequest) {
  ...
  const year = searchParams.get("year") ?? searchParams.get("semester");
...
const createMaterialSchema = z.object({
  ...
  subjectId: z.string().min(1, "请选择课程"),
  semester: z.string().regex(/^\d{4}$/, "年份请填写 4 位数字"),
...
export async function POST(request: Request) {
  ...
  const { title, description, subjectId, semester, fileUrl, fileName, fileSize, fileType } =
    parsed.data;
